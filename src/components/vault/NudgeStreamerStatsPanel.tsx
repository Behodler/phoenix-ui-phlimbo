...
/**
 * Days of runway actually left: the undistributed buffer divided by the rate
 * it is draining at. This is deliberately NOT the configured window — every
 * `collectNudge` re-sizes `rewardPerSecond` from the whole buffer over a fresh
 * window, so a top-up mid-stream stretches the real runway past the setting,
 * and `batchMint` pulls shorten it.
 */
function formatDaysLeft(bufferRaw: bigint, rewardPerSecondRaw: bigint): string {
  if (rewardPerSecondRaw === 0n) return '—';
  if (bufferRaw === 0n) return '0';
  // buffer / (rewardPerSecond / PRECISION) = buffer * PRECISION / rewardPerSecond.
  const secondsLeft =
    (bufferRaw * 10n ** BigInt(STREAM_PRECISION_DECIMALS)) / rewardPerSecondRaw;
      ...
      {isUnavailable || !minterAddress ? (
        ...
        <>
              ...
              <tbody>
                {tokens.map((token) => (
                  <tr key={token.address} className="border-t border-border">
                    ...
                    <td className="py-2 pl-2 text-right font-mono text-foreground">
                      {formatDaysLeft(token.bufferRaw, token.rewardPerSecondRaw)}
                    </td>
                    <td className="py-2 pl-2 text-right font-mono text-foreground">
                      {formatPerSecond(token.rewardPerSecondRaw, token.decimals)}
                    </td>
                    <td className="py-2 pl-2 text-right font-mono text-foreground">
          ...
          <p className="text-xs text-muted-foreground mt-3 pt-3 border-t border-border">
            ...
            <strong>Days left</strong> is the real runway,{' '}
            <code>buffer ÷ rewardPerSecond</code>, which drifts from the window
            ...
            (the 1e18 is the streamer&apos;s fixed-point{' '}
            <code>PRECISION</code>, not decimals). <strong>Remaining</strong> is
            the undistributed <code>buffer</code> still held by the streamer —
            it already includes the accrued-but-unsettled slice that{' '}
            <code>pendingStream</code> reports. A row reads{' '}
